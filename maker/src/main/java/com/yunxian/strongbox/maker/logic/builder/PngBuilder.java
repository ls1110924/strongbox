...
public class PngBuilder implements IBuilder<PngModel> {
    ...
    @Override
    public boolean build(@NotNull String dest, @NotNull PngModel model, @NotNull SecretKeyModel keyModel) {
        if (!keyModel.isLegal()) {
            throw new IllegalArgumentException("the params char is illegal, only support print ascii, plz check");
        }
        byte[] paramsBytes = JSON.toJSONString(keyModel.getParamKeys()).getBytes(StandardCharsets.UTF_8);
